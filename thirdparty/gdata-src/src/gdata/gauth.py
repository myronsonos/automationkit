...
from urllib.parse import urlencode, quote, unquote, urlparse, urlunparse, parse_qsl, quote_plus, unquote_plus
...
def generate_client_login_request_body(email, password, service, source,
  ...
  return urlencode(request_fields)
...
def build_oauth_base_string(http_request, consumer_key, nonce, signaure_type,
  ...
  for key in sorted_keys:
    pairs.append('%s=%s' % (quote(key, safe='~'),
                            quote(params[key], safe='~')))
  # We want to escape /'s too, so use safe='~'
  all_parameters = quote('&'.join(pairs), safe='~')
...
def generate_hmac_signature(http_request, consumer_key, consumer_secret,
  ...
  if token_secret is not None:
    hash_key = '%s&%s' % (quote(consumer_secret, safe='~'),
                          quote(token_secret, safe='~'))
  else:
    hash_key = '%s&' % quote(consumer_secret, safe='~')
  try:
    import hashlib
...
def generate_auth_header(consumer_key, timestamp, nonce, signature_type,
  ...
  pairs = [
      '%s="%s"' % (
          k, quote(v, safe='~')) for k, v in params.iteritems()]
...
def oauth_token_info_from_body(http_body):
  ...
  for pair in http_body.split('&'):
    if pair.startswith('oauth_token='):
      token = unquote(pair[len('oauth_token='):])
    if pair.startswith('oauth_token_secret='):
      token_secret = unquote(pair[len('oauth_token_secret='):])
...
def oauth_token_info_from_url(url):
  ...
  if 'oauth_token' in url.query:
    token = unquote(url.query['oauth_token'])
  if 'oauth_verifier' in url.query:
    verifier = unquote(url.query['oauth_verifier'])
...
class OAuth2Token(object):
  ...
  def generate_authorize_url(self, redirect_uri='urn:ietf:wg:oauth:2.0:oob',
    ...
    query.update(kwargs)
    parts = list(urlparse(self.auth_uri))
...
def _join_token_parts(*args):
  ...
  """
  return '|'.join([quote_plus(a or '') for a in args])
...
def _split_token_parts(blob):
  ...
  """
  return [unquote_plus(part) or None for part in blob.split('|')]
