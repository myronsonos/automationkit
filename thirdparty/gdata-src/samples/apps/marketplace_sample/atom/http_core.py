...
def get_headers(http_response):
  ...
  The HTTPResponse object in 2.2 and 2.3 does not have a getheaders
...
class HttpClient(object):
  """Performs HTTP requests using the http module."""
  debug = None
  ...
  def _get_connection(self, uri, headers=None):
    ...
    if uri.scheme == 'https':
      if not uri.port:
        connection = HTTPSConnection(uri.host)
      else:
        connection = HTTPSConnection(uri.host, int(uri.port))
    else:
      if not uri.port:
        connection = HTTPConnection(uri.host)
      else:
        connection = HTTPConnection(uri.host, int(uri.port))
    return connection

  def _http_request(self, method, uri, headers=None, body_parts=None):
    """Makes an HTTP request using the http module.
...
class ProxiedHttpClient(HttpClient):
  ...
  def _get_connection(self, uri, headers=None):
    ...
    if uri.scheme == 'https':
      ...
      return HTTPConnection(proxy_uri.host, int(proxy_uri.port))
    return None
