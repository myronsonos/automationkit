...
"""HttpClients in this module use http module to make HTTP requests.

This module make HTTP requests based on http module, but there are environments
in which an http module based approach will not work (if running in Google App
...
class HttpClient(atom.http_interface.GenericHttpClient):
  ...
  def request(self, operation, url, data=None, headers=None):
    ...
    # HTTPConnection.putrequest adding
  ...
  def _prepare_connection(self, url, headers):
    ...
    if url.protocol == 'https':
      if not url.port:
        return HTTPSConnection(url.host)
      return HTTPSConnection(url.host, int(url.port))
    else:
      if not url.port:
        return HTTPConnection(url.host)
      return HTTPConnection(url.host, int(url.port))
  ...
  def _prepare_connection(self, url, headers):
    ...
    if not proxy_settings:
      ...
      if url.protocol == 'https':
        ...
        # Initalize http module and replace with the proxy socket.
        connection = HTTPConnection(proxy_url.host)
        ...
        return HTTPConnection(proxy_url.host, int(proxy_url.port))
