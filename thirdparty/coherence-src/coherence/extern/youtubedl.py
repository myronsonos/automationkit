...
from urllib.request import HTTPCookieProcessor, ProxyHandler, Request, URLError
...
class FileDownloader(object):
    ...
    def process_info(self, info_dict):
        ...
        if self.params.get('simulate', False):
            ...
            except (OSError, IOError, URLError, HTTPException, socket.error) as err:
...
class YoutubeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def gotPage(result, format_param, video_extension):
            ...
            video_title = re.sub(r'(?u)&(.+?);', self.htmlentity_transform, video_title)
            video_title = video_title.replace(os.sep, u'%')

            # simplified title
            simple_title = re.sub(r'(?u)([^%s]+)' % simple_title_chars, r'_', video_title)
...
class MetacafeIE(InfoExtractor):
    ...
    def _real_initialize(self):
        # Retrieve disclaimer
        request = Request(self._DISCLAIMER, None, std_headers)
        try:
            self.report_disclaimer()
            disclaimer = urlopen(request).read()
        except (URLError, HTTPException, socket.error) as err:
        ...
        request = Request(self._FILTER_POST, urllib.urlencode(disclaimer_form), std_headers)
        try:
    ...
    def _real_extract(self, url):
        ...
        request = Request('http://www.metacafe.com/watch/%s/' % video_id)
        try:
...
class YoutubeSearchIE(InfoExtractor):
    ...
    def _download_n_results(self, query, n):
        ...
        while True:
            ...
            request = Request(result_url, None, std_headers)
            try:
...
class YoutubePlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        while True:
            ...
            request = Request(self._TEMPLATE_URL % (playlist_id, pagenum), None, std_headers)
            try:
                page = urlopen(request).read()
            except (URLError, HTTPException, socket.error) as err:
                self._downloader.trouble(u'ERROR: unable to download webpage: %s' % str(err))
