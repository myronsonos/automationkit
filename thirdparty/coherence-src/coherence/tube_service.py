...
from urllib.parse import quote_plus, unquote_plus, urlunsplit
                ...
                for item in didl.getItems():
                    ...
                    for res in item.res:
                        ...
                        if remote_protocol == 'http-get' and remote_network == '*':
                            quoted_url = quote_plus(res.data)
                            print(("modifying", res.data))
                            res.data = urlunsplit(('http', self.service.device.external_address, 'mirabeau', quoted_url, ""))
                            print(("--->", res.data))
                            new_res.append(res)
                            changed = True
                    item.res = new_res
