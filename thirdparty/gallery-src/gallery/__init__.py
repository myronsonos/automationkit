...
def handle_command_thumb(
    ...
    force = False
    if 'gallery_still' in internal_vars:
        log.info(
            "Making video stills"
        )
    if 'gallery_thumb' in internal_vars:
        if 'gallery_still' in internal_vars and \
           'gallery_reduced' in internal_vars:
            log.info(
                "Making image and video thumbnails"
            )
        elif 'gallery_still' in internal_vars:
            log.info(
                "Making video thumbnails"
            )
        elif 'gallery_reduced' in internal_vars:
            log.info(
                "Making image thumbnails"
            )
    if 'gallery_reduced' in internal_vars:
        log.info(
            "Making reduced images"
        )
    if 'gallery_meta' in internal_vars:
        log.info(
            "Making meta files"
        )
    if 'gallery_exif' in internal_vars:
        log.info(
            "Extracting EXIF data"
        )
    if 'gallery_h264' in internal_vars:
        log.info(
            "Making h264 videos"
        )
    ...
    for dirpath, dirnames, filenames in os.walk(source):
        ...
        for filename in filenames:
            ...
            if 'gallery_meta' in internal_vars and not os.path.exists(os.path.join(dest, 'meta', path[len(source)+1:])):
                    ...
                    if force or (not os.path.exists(dst)) or \
                       ...
                       (os.stat(source)[8] > os.stat(dst)[8]):
                        ...
                        if retcode != 0:
                            ...
                            log.warning('out: %s', out)
                            log.warning('err: %s', err)
            if 'gallery_exif' in internal_vars and \
...
def handle_command_metadata(
    ...
    if 'format' not in internal_vars:
        ...
        for filename in filenames:
            ...
            if filename.split('.')[-1].lower() in ['cr2', 'png', 'jpg', 'jpeg']:
                ...
                if not os.path.isdir(os.path.join(dirpath, filename)):
                    path = os.path.relpath(os.path.join(picpath, filename), source_1024)
                    photos.append(path)
            else:
                vidpath = dirpath.replace(meta, source_h264)
                if not os.path.isdir(os.path.join(vidpath, filename)):
                    path = os.path.relpath(os.path.join(vidpath, filename), source_h264)
                    photos.append(path)
    # Last one
    if photos:
        write(output, sort(photos))
